...
Plue2011 <- DAT.T %>%
  ...
  mutate(PopID = ifelse(LastPopID == "1", "l", "h")) %>% 
  select(-LastSiteID, -LastPopID)
...
site.info <- LA %>% 
  ...
  mutate(factor.site2 = ifelse(StudyID == "Plue2011PlED" & elevation_site %in% c(626, 600), "low", ifelse(StudyID == "Plue2011PlED" & elevation_site %in% c(1251, 1235), "high", factor.site2))) %>% 
  select(-low.site, -high.site)


# add population as factor (High/Mid/Low)
elev_factor <- read.table("elev_factor.csv", sep=";", header=TRUE)
