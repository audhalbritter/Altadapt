...
# summary table 2
dd2 <- dat2 %>% select(study_type, growthform, family, species, longevity, intro, breed, studysite, StudyID) %>% 
  distinct() %>% 
  arrange(study_type, growthform, longevity, family, species) %>% 
  rename("Growthform" = "growthform", "Family"  = "family", "Species" = "species", "Longevity" = "longevity", "Introduction_status" = "intro", "Breeding_system" = "breed", "Study_type" = "studysite", "Reference" = "StudyID") %>% 
  mutate(Species = gsub("_", " ", Species)) %>% 
  mutate(Study_type = plyr::mapvalues(Study_type, c("Garden", "climate_chamber", "greenhouse"), c("G", "CC", "GH"))) %>% 
  mutate(Breeding_system = plyr::mapvalues(Breeding_system, c("mixed_mating", "outcrossing", "inbreeding", "NA"), c("mixed mating", "outcrossing", "inbreeding", "-"))) %>% 
  select(-study_type)

write.csv(dd, "dd.csv", row.names = FALSE)
write.csv(dd2, "TableS6.csv", row.names = FALSE)
...
library("broom")
# Correlation between abs. slopes and T Range
res <- dat4 %>% 
  group_by(var) %>% 
  do(fit = lm(abs.slopes ~ r.temp + I(r.temp^2), data = .))

tidy(res, fit)
glance(res, fit)

hhh <- dat4 %>% filter(var == "phenology")
fit = lm(abs.slopes ~ r.temp + I(r.temp^2), data = hhh)
summary(fit)
